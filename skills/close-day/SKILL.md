...
- **My Top 3:** Left blank by the close-day seed. The next morning, open-day pre-fills slot 1 with the AI #1 pick as a demonstrated, one-click-undoable example (Plan-your-day redesign) — the builder may adopt, modify, or completely replace it.
- **The close-day seed never writes a `### Bonus` pile.** It writes only the `### AI Suggested: …` candidate pool + `## Carrying Over`; every unfinished item surfaces the next morning as a suggestion row with one-click Top 3 / Bonus / Defer buttons, so nothing gets trapped anywhere. (The only Bonus line present at dawn is the single pre-selected example open-day writes — the highest-progress carry-over, one click to un-take.) The `### AI Suggested: Top 3` picks are the AI's 3 strongest; genuine extra carry-overs still surface via `## Carrying Over`, which the companion folds into the same suggestion list.
