...
- **Step 6**: Write the daily note with **empty Top 3 slots AND an empty Bonus section** (`1. [ ]`, `2. [ ]`, `3. [ ]` for Top 3; nothing under `### Bonus`). Include habits, calendar, and the standard template. **Do NOT fill in Top 3 or Bonus — that's the companion's job.** Everything the AI knows about (carry-overs, close-day seeds, generated ideas) goes into the **candidate pool** — the `### AI Suggested: …` sections the companion renders as the "Suggestions & carry-overs" grid, where every row has a one-click **Top 3 / Bonus / Defer / Delete** control.
  ...
  **Pre-select two examples** so the plan page opens with the pattern demonstrated (people learn they can choose by seeing one already chosen): write the AI #1 pick into `### My Top 3` slot 1, and the highest-progress unfinished carry-over into `### Bonus` slot 1. Rules: the slot text must be the **exact title as the companion displays it** — no checkbox leak, no `**bold**`, no `— rationale` tail — so the suggestion row lights up as taken (the companion matches on text); keep the item's `<!--e:X-->` marker, and on the Bonus pick also keep its `<!--p:NN-->` progress marker so the Command Center circle starts where yesterday stopped. Slots 2-3 stay empty (`2. [ ]`, `3. [ ]`). No carry-over with progress → pre-select only the Top 3 example. One click un-takes either, so this is a suggestion, not a decision made for the builder.
